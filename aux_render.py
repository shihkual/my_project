...
def render_with_katic(job):
    ...
    def plot(i):
        ...
        for k in k_atic:
            ...
            fig.colorbar(sm, cax=cax).set_label(fr"$|\psi'_{k}(i)|$")
...
def render_chosen(job):
    ...
    chosen_frame = np.linspace(0, traj_length-1, 4).astype(np.int32)
    _, k_atic = get_frame_and_katic(job)
    ...
    for i in chosen_frame:
        ...
        fig, ax = plt.subplots(1, 1, figsize=(9, 5))
        plot(frame)
        fig.savefig(f"{LABEL}_frame{i}.png", dpi=500)
